...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        lbs_results = lbs(betas, full_pose, self.v_template,
                            self.shapedirs, self.posedirs,
                            self.J_regressor, self.parents,
                            self.lbs_weights, pose2rot=pose2rot,
                            return_affine_mat = True,
                            return_shape_disp=return_shape_disp)
        if not return_shape_disp:
            vertices, joints, A = lbs_results
        else:
            vertices, joints, A, shape_disp, pose_disp = lbs_results
        ...
        if not return_shape_disp:
            ...
            return output, shape_disp, pose_disp
