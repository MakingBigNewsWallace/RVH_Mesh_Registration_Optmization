...
video_id = "avatarrex_zzr"
# video_id = "avatarrex_lbn2"
# pose_id = "AMASS__CMU_06_06_13_stageii_moving_view"
# pose_id = "AMASS__CMU_131_131_03_stageii_moving_view"
pose_id = "AMASS__CMU_06_06_13_stageii_aligned_moving_view"

# root = f"/root/autodl-tmp/datazy/CVPR25/GaussianAvatar/gs_data/avatarrex/{video_id}/thuman4__pose_00_free_view/train"
# root = f"/root/autodl-tmp/datazy/CVPR25/GaussianAvatar/gs_data/avatarrex/{video_id}/AMASS__CMU_06_06_13_stageii_moving_view/train"
# root = f"/root/autodl-tmp/datazy/CVPR25/GaussianAvatar/gs_data/avatarrex/{video_id}/AMASS__CMU_131_131_03_stageii_moving_view/train"
root = f"/root/autodl-tmp/datazy/CVPR25/GaussianAvatar/gs_data/avatarrex/{video_id}/{pose_id}/train"

image_dir = os.path.join(root, "images")
cam_dir = os.path.join(root, "cam_parms")

smplx_params = torch.load(os.path.join(root, "smpl_parms.pth"))
betas = smplx_params["beta"]
transl = smplx_params["trans"]
global_orient = smplx_params["body_pose"][:, 0:3]
body_pose = smplx_params["body_pose"][:, 3:66]
...
model_path = "smpl_assets/smplx_model"
...
for framed_index in tqdm.tqdm(range(len(image_list))):
    cam_path = os.path.join(cam_dir, cam_list[framed_index])
    ...
    curr_global_orient = global_orient[framed_index:framed_index+1].float().to(device)
    curr_body_pose = body_pose[framed_index:framed_index+1].float().to(device)
    curr_betas = betas.float().to(device)
    curr_transl = transl[framed_index:framed_index+1].float().to(device)
    curr_left_hand_pose = left_hand_pose[framed_index:framed_index+1].float().to(device)
    curr_right_hand_pose = right_hand_pose[framed_index:framed_index+1].float().to(device)
    ...
    ## save image
    output_path = f"./output/avatar/{video_id}/{pose_id}/{id_str}.jpg"
...
output_video_path = os.path.join(output_dir, f"render_smpl_{video_id}_{pose_id}.mp4")
